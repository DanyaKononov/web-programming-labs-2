...
def final():
    try:
        ...
        if age < 18:
            if gender == 'male':
                message = f"Поздравляю тебя, {name}! Ты еще такой маленький)) желаю тебе всего самого наилучшего!)"
            else:
                message = f"Поздравляю тебя, {name}! Ты еще такая маленькая)) желаю тебе всего самого наилучшего!)"
        else:
            if gender == 'male':
                message = f"Поздравляю вас, {name}! Ты уже такой взрослый! желаю тебе всего самого наилучшего!"
            else:
                message = f"Поздравляю вас, {name}! Ты уже такой взрослый! желаю тебе всего самого наилучшего!"
        ...
        if preference1 == 'something_tasty':
            if preference2 == 'sweet':
                image = 'Sweety.jpg'
                message += "Я хочу тебе подарить вкусненькие конфеты)"
            else:
                image = 'lopshibakka.jpg'
                message += "Я хочу тебе подарить вкусненькие Лопшибакку))"
        else:
            if preference2 == 'refined':
                image = 'planckton.jpg'
                message += "Я хочу подарить тебе интересные ботиники))"
            else:
                image = 'fire.jpg'
                message += "Я хочу подарить тебе бенгальские огни))"
